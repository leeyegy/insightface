...
class Bottleneck(namedtuple('Block', ['in_channel', 'depth', 'stride','set_channel'])):
    '''A named tuple describing a ResNet block.'''

def get_block(in_channel, depth, num_units, stride=2):
    return [Bottleneck(in_channel, depth, stride,in_channel)] + [Bottleneck(depth, depth, 1,0) for i in range(num_units - 1)]
...
class Backbone(Module):
    def __init__(self, num_layers, drop_ratio, mode='ir'):
        ...
        self.input_layer = Sequential(Conv2d(3, 64, (3, 3), 1, 1, bias=False),
                                      BatchNorm2d(64,eps=2e-5),
                                      PReLU(64))
        self.output_layer = Sequential(BatchNorm2d(512,eps=2e-5),
                                       Dropout(drop_ratio),
                                       Flatten(),
                                       Linear(512 * 7 * 7, 512),
                                       BatchNorm1d(512,eps=2e-5))
        modules = []
        for block in blocks:
